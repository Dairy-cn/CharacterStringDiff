...
public class DiffMatchPatch {
	...
	protected LinkedList<Diff> diffCompute(String text1, String text2, boolean checklines) {
		...
		if (hm != null) {
			...
			LinkedList<Diff> diffsA = diffMain(text1_A, text2_A, checklines);
			LinkedList<Diff> diffsB = diffMain(text1_B, text2_B, checklines);
			// Merge the results.
			diffs = diffsA;
	...
	protected LinkedList<Diff> diffMap(String text1, String text2) {
		...
		for (int d = 0; d < max_d; d++) {
			...
			if (DiffTimeout > 0 && System.currentTimeMillis() > ms_end) {
				return null;
			}
	...
	public int diffCommonPrefix(String text1, String text2) {
		...
		int n = Math.min(StringUtils.isEmpty(text1) ? 0 : text1.length(), StringUtils.isEmpty(text2) ? 0 : text2.length());
		for (int i = 0; i < n; i++) {
...
//			&para;
			switch (aDiff.operation) {
				case INSERT:
					html.append("<INS STYLE=\"background:#0000FF;\" TITLE=\"i=").append(i)
							.append("\">").append(text).append("</INS>");
					break;
				case DELETE:
					html.append("<DEL STYLE=\"background:#FF0000;\" TITLE=\"i=").append(i)
		...
		for (Diff aDiff : diffs) {
			...
			switch (aDiff.operation) {
				case INSERT:
					...
					postpatch_text = postpatch_text.substring(0, charCount2)
							+ aDiff.text + postpatch_text.substring(charCount2);
					break;
				case DELETE:
					patch.length1 += aDiff.text.length();
					patch.diffs.add(aDiff);
					postpatch_text = postpatch_text.substring(0, charCount2)
							+ postpatch_text.substring(charCount2 + aDiff.text.length());
					break;
				case EQUAL:
							...
							if (aDiff.operation != Operation.EQUAL) {
								...
								if (aDiff.operation == Operation.INSERT) {
									...
									text = text.substring(0, startLoc + index2)
											+ text.substring(startLoc + diff_xIndex(diffs,
			...
			while (!bigpatch.diffs.isEmpty()) {
				...
				while (!bigpatch.diffs.isEmpty()
					...
					if (diff_type == Operation.INSERT) {
						...
						empty = false;
						patch.diffs.add(new Diff(diff_type, diffText));
	 ...
	 */
	private String unescapeForEncodeUriCompatability(String str) {
	...
	public String getHtmlDiffString(String text1, String text2) {
		return diffPrettyHtml(diffMain(text1, text2));
	}
